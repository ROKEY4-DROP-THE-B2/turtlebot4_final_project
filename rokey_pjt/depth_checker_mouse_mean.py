...
class DepthChecker(Node):
    ...
    # mouse_callback 함수 수정 예시
    def mouse_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN and self.depth_mm is not None:
            # 클릭 지점 주변의 3x3 픽셀 영역에서 깊이값 추출
            # 경계 체크 필요
            if y > 0 and y < self.depth_mm.shape[0]-1 and x > 0 and x < self.depth_mm.shape[1]-1:
                depth_region = self.depth_mm[y-1:y+2, x-1:x+2]
                # 중간값으로 깊이 계산
                distance_mm = np.median(depth_region)
                distance_m = distance_mm / 1000.0
                self.get_logger().info(f"Clicked at (u={x}, v={y}) → Distance = {distance_m:.2f} meters (Median of 3x3 region)")
